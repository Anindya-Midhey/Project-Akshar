...
const createPage = (data, name) => ({
  ...
  // ── Module 1 state ──────────────────────────────────────────────────────
  // m1Processed:   true only after /m1/process succeeds for THIS page.
  //                Controls Revert button visibility (image-specific).
  m1Processed: false,
  m1Loading: false,   // true while SAM is running (spinner)
  m1ProcessedPath: null,    // abs path to M1 output file
  m1ProcessedPreview: null,    // base64 data-URI of M1 output
});
...
export default function App() {
  ...
  const [dewarpMode, setDewarpMode] = useState('manual'); // 'auto' | 'manual'
  ...
  // -----------------------------------------------------------------------
  // Module 1 — Background processing + Revert
  // -----------------------------------------------------------------------

  /**
   * runModule1ForPage
   * -----------------
   * Fires the Module 1 SAM pipeline for a page in the background.
   * Does NOT block the workbench from opening — the user sees the original
   * image immediately and the canvas updates when SAM finishes.
   *
   * If the pipeline returns 2 pages (double-page spread), both are inserted
   * into the pages array, replacing the single original slot.
   *
   * @param {number} pageIndex  Index of the page in `pages` state.
   * @param {string} imagePath  Server-side path to the uploaded image.
   */
  const runModule1ForPage = useCallback(async (pageIndex, imagePath) => {
    // Mark this page as "M1 loading" (shows spinner in Controls)
    setPages(prev => prev.map((p, i) =>
      i === pageIndex ? { ...p, m1Loading: true } : p
    ));

    try {
      const res = await module1Process(imagePath, 'color');

      if (res.status !== 'success' || !res.pages?.length) {
        throw new Error('Module 1 returned no pages');
      }

      const m1Pages = res.pages;

      if (m1Pages.length === 1) {
        // ── Single page: update in place ─────────────────────────────────
        const pg = m1Pages[0];
        setPages(prev => prev.map((p, i) => {
          ...
          return {
            ...p,
            m1Loading: false,
            m1Processed: true,          // ← enables Revert button
            m1ProcessedPath: pg.image_path,
            m1ProcessedPreview: pg.preview,
            // Switch canvas to M1 result
            currentPath: pg.image_path,
            currentPreview: pg.preview,
            imageWidth: pg.width,
            imageHeight: pg.height,
            activeFilter: 'Module 1 (SAM)',
            history: [...p.history, {
              action: 'Module 1',
              preview: pg.preview,
              path: pg.image_path,
        ...
        toast('Module 1 processing complete', 'success');

      } else {
        // ── Double-page spread: replace 1 slot with 2 new pages ──────────
        const newSlots = m1Pages.map((pg, idx) => ({
          id: Date.now() + Math.random(),
          filename: `Page ${idx + 1} (M1)`,
          originalPath: pg.image_path,   // M1 output IS the original for M2 ops
          originalPreview: pg.preview,
          currentPath: pg.image_path,
          currentPreview: pg.preview,
          imageWidth: pg.width,
          imageHeight: pg.height,
          corners: initCorners(pg.width, pg.height),
          activeFilter: 'Module 1 (SAM)',
          m1Processed: true,
          m1Loading: false,
          m1ProcessedPath: pg.image_path,
          m1ProcessedPreview: pg.preview,
          history: [{ action: 'Module 1 (split)', preview: pg.preview, path: pg.image_path, width: pg.width, height: pg.height }],
        }));
        ...
        toast(`Module 1 split into ${m1Pages.length} pages`, 'success');
      }

    } catch (err) {
      console.error('[M1] Processing failed:', err);
      // Mark m1Loading=false and leave the original image intact
      setPages(prev => prev.map((p, i) =>
        i === pageIndex ? { ...p, m1Loading: false } : p
      ));
      toast(`Module 1 failed: ${err.message || 'unknown error'}`, 'error');
    }
  }, [toast]);
   ...
   * handleRevertM1
   * --------------
   * Instantly restores the original uploaded image for the given page.
   * Zero API calls — uses cached originalPath / originalPreview.
   * Sets m1Processed=false so the Revert button disappears.
   */
  const handleRevertM1 = useCallback((pageIndex) => {
    setPages(prev => prev.map((p, i) => {
      if (i !== pageIndex) return p;
      return {
        ...p,
        currentPath: p.originalPath,
        currentPreview: p.originalPreview,
        imageWidth: p.history[0]?.width ?? p.imageWidth,
        imageHeight: p.history[0]?.height ?? p.imageHeight,
        activeFilter: null,
        m1Processed: false,   // ← hides Revert button immediately
        m1ProcessedPath: null,
        m1ProcessedPreview: null,
        history: [...p.history, {
          action: 'Revert to Original',
          preview: p.originalPreview,
          path: p.originalPath,
          width: p.history[0]?.width ?? p.imageWidth,
          height: p.history[0]?.height ?? p.imageHeight,
        }],
      };
    }));
    toast('Reverted to original image', 'info');
  }, [toast]);
  ...
  const handleUpload = useCallback(async (files) => {
    ...
    try {
      ...
      if (ext === 'pdf') {
        ...
        // Add pages directly — Module 1 (SAM) is NOT run automatically.
        // The original uploaded image lands in the workbench as-is.
  ...
  const handleCornersDetect = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await cornersDetect(activePage.currentPath);
      if (data.corners) {
  ...
  const handleCornersApplyAll = useCallback(async () => {
    ...
    try {
      for (let i = 0; i < snap.length; i++) {
        ...
        // Auto detect corners first for each page
        const detectData = await cornersDetect(snap[i].currentPath);
  ...
  const handleDewarpAutoAll = useCallback(async () => {
    ...
    try {
      for (let i = 0; i < snap.length; i++) {
        setBatchProgress(i + 1);
        if (snap[i].activeFilter === 'Auto Dewarp') continue; // Skip if already applied
        const data = await dewarpImageAuto(snap[i].currentPath);
        updatePage(i, {
          currentPreview: data.preview, currentPath: data.image_path,
          imageWidth: data.width, imageHeight: data.height,
          activeFilter: 'Auto Dewarp',
          history: [...snap[i].history,
            { action: 'Auto Dewarp', preview: data.preview, path: data.image_path, width: data.width, height: data.height }],
        });
      }
      toast(`Auto dewarp applied to all ${snap.length} pages`, 'success');
    } catch (e) { toast(e.message || 'Batch dewarp failed', 'error'); }
    finally { setBatchLoading(false); setBatchProgress(0); }
  }, [pages, updatePage, toast]);
              ...
              >
                ...
                <div className="wf-option-body">
                  ...
                  <div className="wf-option-flow">
                    ...
                    <span className="wf-flow-badge wf-flow-skip">Module 2</span>
                    <span className="wf-flow-arrow">→</span>
              ...
              >
                ...
                <div className="wf-option-body">
                  ...
                  <div className="wf-option-flow">
                    ...
                    <span className="wf-flow-arrow">→</span>
                    <span className="wf-flow-badge wf-flow-active">Module 2</span>
                    <span className="wf-flow-arrow">→</span>
                    <span className="wf-flow-badge wf-flow-active">Module 3</span>
                    <span className="wf-flow-arrow">→</span>
                    <span className="wf-flow-badge wf-flow-active">Module 4</span>
                ...
                {activePage ? (
                  ...
                  >
                    <CanvasEditor
                      ...
                      isWarpMode={false}
                      dewarpGrid={dewarpOpen ? dewarpGrid : null}
                      onDewarpGridChange={setDewarpGrid}
            ...
            <div className="editor-sidebar">
              ...
              {activePage && (
                <Controls
                  ...
                  m1Processed={activePage.m1Processed}
                  m1Loading={activePage.m1Loading}
                  onRevertM1={() => handleRevertM1(activePageIndex)}
                />
              )}
