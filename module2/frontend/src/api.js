 ...
 * All calls to the FastAPI backend on port 8000.
 */
...
export async function dewarpImageAuto(imagePath) {
  return apiPost('/dewarp/auto', { image_path: imagePath });
}
...
export async function exportPdfBbox(pdfPath, blocks, ocr_dpi = 300) {
  const res = await fetch(`${API_BASE}/export/pdf-bbox`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function cornersDetect(imagePath) {
  return apiPost('/corners/detect', { image_path: imagePath });
}
...
/**
 * Run the full Module 1 pipeline on an already-uploaded image.
 *
 * Pipeline: resize → page_split_check → SAM_extract → remove_border
 *           → shadow_removal → enhance (color)
 *
 * Returns:
 *   {
 *     status: "success",
 *     page_count: 1 | 2,
 *     pages: [{ image_path, preview, width, height }, ...],
 *     steps_applied: [...]
 *   }
 *
 * NOTE: SAM inference can take 10–30 s on CPU.  Call this non-blocking
 *       (fire-and-forget after upload) so the workbench opens immediately.
 */
export async function module1Process(imagePath, mode = 'color') {
  return apiPost('/m1/process', { image_path: imagePath, mode });
}
