...
export default function Controls({
  ...
  return (
    <div className="st-panels-block">
      ...
      {/* ── Module 1 Status / Revert (image-specific) ──────────────── */}
      {/* Shown only when m1Loading=true OR m1Processed=true for THIS page. */}
      {(m1Loading || m1Processed) && (
        <div className="st-panel open"
          style={{ borderLeft: '2px solid rgba(124,77,255,0.6)' }}>
          <div className="st-panel-header open" style={{ cursor: 'default' }}>
            <span className="st-panel-num" style={{ color: '#b39ddb' }}>M1</span>
            <span className="st-panel-title" style={{ color: '#b39ddb' }}>Module 1</span>
          </div>
          <div className="st-panel-body">

            {/* Loading indicator while SAM runs */}
            {m1Loading && (
              <div style={{
                display: 'flex', alignItems: 'center', gap: 8,
                fontSize: 11, color: 'var(--color-text-3)', marginBottom: 8,
              }}>
                <span className="spinner" />
                <span>SAM processing… (this may take 20–30 s)</span>
              </div>
            )}

            {/* Revert button — only after M1 has successfully processed */}
            {m1Processed && !m1Loading && (
              <button
                id="btn-revert-m1"
                className="btn w-full"
                onClick={onRevertM1}
                style={{
                  background: 'linear-gradient(135deg,rgba(124,77,255,0.25),rgba(90,55,190,0.25))',
