"""
Module 2 - Interactive Image Workbench: Image Enhancement Utilities
...
def _remove_black_border(image: np.ndarray) -> np.ndarray:
    ...
    Module 2 receives manually perspective-cropped pages, not SAM-whitened
...
def _resize_to_module1_width(image: np.ndarray, width: int = 800) -> np.ndarray:
...
def _module1_remove_shadow(image: np.ndarray) -> np.ndarray:
    """
...
def _module1_enhance_bw(image: np.ndarray) -> np.ndarray:
    """
    ...
    """
    image = _module1_remove_shadow(image)
...
def apply_otsu_threshold(image: np.ndarray, mode: str = "color") -> np.ndarray:
    ...
    return _module1_enhance_bw(image)
