...
async def detect_document_type(request: DetectRequest):
    ...
    return {
        ...
        "message": (
            ...
            if doc_type == "digital"
            else "Scanned PDF detected — routing through Image Workbench (Module 2 → 3 → 4)."
        )
    }
...
async def extract_pdf_pages_endpoint(request: ExtractPagesRequest):
    ...
    Returns ordered list of image paths for feeding into Module 2.
    """
    if not os.path.exists(request.pdf_path):
        raise HTTPException(status_code=404, detail="PDF not found")
...
async def convert_images_to_pdf(request: ConvertToPdfRequest):
    ...
    Call this after Module 2 processing to produce the clean output PDF.
    """
